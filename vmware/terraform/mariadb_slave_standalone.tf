...
#Variable : Node01_mariadb_root_password
variable "Node01_mariadb_root_password" {
  type        = "string"
  description = "The mariadb root password."
}

#Variable : Node01_mariadb_server_id
variable "Node01_mariadb_server_id" {
  type        = "string"
  description = "MariaDB server ID. Slave should not be 1"
  default     = "2"
}

#Variable : Node01_mariadb_server_id
variable "Node01_mariadb_master_address" {
  type        = "string"
  description = "MariaDB master server address"
  default     = ""
}

variable "Node01_mariadb_master_port" {
  type        = "string"
  description = "MariaDB master server port"
  default     = "2"
}

variable "Node01_mariadb_master_username" {
  type        = "string"
  description = "MariaDB master server username"
  default     = ""
}

variable "Node01_mariadb_master_password" {
  type        = "string"
  description = "MariaDB master server password"
  default     = ""
}

variable "Node01_mariadb_master_rep_username" {
  type        = "string"
  description = "MariaDB master server username"
  default     = ""
}

variable "Node01_mariadb_master_rep_password" {
  type        = "string"
  description = "MariaDB master server password"
  default     = ""
}
...
resource "null_resource" "Node01_Link" {
  depends_on = ["vsphere_virtual_machine.Node01", "camc_softwaredeploy.Node01_mariadb"]
  ...
  provisioner "file" {
    destination = "master_add_user.sql"
    content     = <<EOF
create user '${var.Node01_mariadb_master_rep_username}'@'${var.Node01_ipv4_address}' identified by '${var.Node01_mariadb_master_rep_password}';
grant replication slave on *.* to '${var.Node01_mariadb_master_rep_username}'@'${var.Node01_ipv4_address}';
flush privileges;
EOF
  }

    provisioner "file" {
    destination = "slave_start.sql"
    content     = <<EOF
...
EOF
  ...
  provisioner "remote-exec" {
    inline = [
      "mysql -h ${var.Node01_mariadb_master_address} -P ${var.Node01_mariadb_master_port} -u${var.Node01_mariadb_master_username} -p${var.Node01_mariadb_master_password} < master_add_user.sql",
      "mysql -uroot -p${var.Node01_mariadb_root_password} < slave_start.sql"
    ]
  }
}
